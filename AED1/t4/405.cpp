...
bool checkupdate(const std::vector<bool>& nodeupdated) {
    return std::find(nodeupdated.begin(), nodeupdated.end(), false) == nodeupdated.end();
}
        ...
        while (c != 0) {
            ...
            for (const auto& pair : pairs) {
                if ((nodeupdated[pair.nod1-1] || nodeupdated[pair.nod2-1]) &&
                    std::find(pair.meetslots.begin(), pair.meetslots.end(),
                        slot % slotsperday)
                    != pair.meetslots.end())
                {
                    if (nodeupdated[pair.nod1-1] != nodeupdated[pair.nod2-1]) {
                        c++;
                        nodeupdated[pair.nod1-1] = nodeupdated[pair.nod2-1] = true;
                    }
                }
            }
        }
