...
void MyGLWidget::paintGL() {
    ...
    glBindVertexArray(VAO_model[mid]);
    glDrawArrays(GL_TRIANGLES, 0, m[mid].faces().size() * 3);
}
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *event) {
    ...
    if (event->buttons() == Qt::LeftButton) {
        phi     += (M_PI/180.0f)*(pos.x() - last_pos.x());
        theta   += (M_PI/180.0f)*(pos.y() - last_pos.y());

    }

    emit phiChanged(180.0f * phi / M_PI);
    emit thetaChanged(180.0f * theta / M_PI);
...
void MyGLWidget::creaBuffers() {
    m[0].load("../models/Patricio.obj");
    m[1].load("../models/legoman.obj");

    glGenVertexArrays(2, VAO_model);

    GLuint VBO_model[2][2];
    glGenBuffers(2, VBO_model[0]);
    glGenBuffers(2, VBO_model[1]);

    /* Patricio */
    glBindVertexArray(VAO_model[0]);

    glBindBuffer(GL_ARRAY_BUFFER, VBO_model[0][0]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * m[0].faces().size() * 3 * 3,
        m[0].VBO_vertices(), GL_STATIC_DRAW);

    glVertexAttribPointer(vertexLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(vertexLoc);

    glBindBuffer(GL_ARRAY_BUFFER, VBO_model[0][1]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * m[0].faces().size() * 3 * 3,
        m[0].VBO_matdiff(), GL_STATIC_DRAW);

    glVertexAttribPointer(colorLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(colorLoc);

    /* Patricio */
    glBindVertexArray(VAO_model[1]);

    glBindBuffer(GL_ARRAY_BUFFER, VBO_model[1][0]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * m[1].faces().size() * 3 * 3,
                 m[1].VBO_vertices(), GL_STATIC_DRAW);

    glVertexAttribPointer(vertexLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
    glEnableVertexAttribArray(vertexLoc);

    glBindBuffer(GL_ARRAY_BUFFER, VBO_model[1][1]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * m[1].faces().size() * 3 * 3,
                 m[1].VBO_matdiff(), GL_STATIC_DRAW);
...
void MyGLWidget::init_camera() {
    ...
    glm::mat4 proj;
    if (projid == 0)
        proj = glm::perspective(zoom * fov, ratio, d - r, d + r);
    else
        proj = glm::ortho(-4.0f, 4.0f, -4.0f, 4.0f, d - r, d + r);
    glUniformMatrix4fv(projLoc, 1, GL_FALSE, glm::value_ptr(proj));
}

void MyGLWidget::setFov(int fov) {
    this->fov = M_PI*(float)fov/180.0f;
}

void MyGLWidget::setModelPatricio() {
    mid = 0;
}

void MyGLWidget::setModelLegoman() {
    mid = 1;
}

void MyGLWidget::setProjectionPerspective() {
    projid = 0;
    modelBoundinxBox();
}

void MyGLWidget::setProjectionOrthogonal() {
    projid = 1;
    modelBoundinxBox();
}
...
void MyGLWidget::setPhi(int phi) {
    this->phi = M_PI * (float)phi / 180.0f;
}

void MyGLWidget::setTheta(int theta) {
    this->theta = M_PI * (float)theta / 180.0f;
}
